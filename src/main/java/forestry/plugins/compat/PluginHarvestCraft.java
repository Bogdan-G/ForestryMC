...
public class PluginHarvestCraft extends ForestryPlugin {
	...
	private final String farmOrchard_text = "farmOrchard";
	private final String farmWheat_text = "farmWheat";
	...
	protected void registerRecipes() {
		...
		for (String berryName : berries) {
			...
			if (PluginManager.Module.FARMING.isEnabled() && berrySeed != null && berryBlock != null) {
				Farmables.farmables.get(farmWheat_text).add(new FarmableGenericCrop(berrySeed, berryBlock, 7));
				Farmables.farmables.get(farmOrchard_text).add(new FarmableBasicFruit(berryBlock, 7));
			}
		...
		for (String fruitName : fruits) {
			...
			if (PluginManager.Module.FARMING.isEnabled() && fruitSeed != null && fruitBlock != null) {
				Farmables.farmables.get(farmWheat_text).add(new FarmableGenericCrop(fruitSeed, fruitBlock, 7));
				Farmables.farmables.get(farmOrchard_text).add(new FarmableBasicFruit(fruitBlock, 7));
			}
		...
		for (String vegetableName : vegetables) {
			...
			if (PluginManager.Module.FARMING.isEnabled() && vegetableSeed != null && vegetableBlock != null) {
				...
				Farmables.farmables.get(farmOrchard_text).add(new FarmableBasicFruit(vegetableBlock, 7));
			}
		...
		for (String grainName : grains) {
			...
			if (PluginManager.Module.FARMING.isEnabled() && grainSeed != null && grainBlock != null) {
				Farmables.farmables.get(farmWheat_text).add(new FarmableGenericCrop(grainSeed, grainBlock, 7));
				Farmables.farmables.get(farmOrchard_text).add(new FarmableBasicFruit(grainBlock, 7));
			}
		...
		for (String treeFruitName : treeFruits) {
			...
			if (PluginManager.Module.FARMING.isEnabled() && treeFruitBlock != null) {
				Farmables.farmables.get(farmOrchard_text).add(new FarmableBasicFruit(treeFruitBlock, 2));
			}
			if (treeFruit != null) {
		...
		for (String treeName : trees) {
			...
			if (PluginManager.Module.FARMING.isEnabled() && fruitBlock != null) {
				Farmables.farmables.get(farmOrchard_text).add(new FarmableBasicFruit(fruitBlock, 2));
			}
			plants.add(treeName);
		}
		...
		for (String cropName : genericCrops) {
			...
			if (PluginManager.Module.FARMING.isEnabled() && genericCropSeed != null && genericCropBlock != null) {
				Farmables.farmables.get(farmWheat_text).add(new FarmableGenericCrop(genericCropSeed, genericCropBlock, 7));
				Farmables.farmables.get(farmOrchard_text).add(new FarmableBasicFruit(genericCropBlock, 7));
			}
			plants.add(cropName);
		}
		...
		if (PluginManager.Module.FARMING.isEnabled() && mustardCropSeed != null && mustardCropBlock != null) {
			Farmables.farmables.get(farmWheat_text).add(new FarmableGenericCrop(mustardCropSeed, mustardCropBlock, 7));
			Farmables.farmables.get(farmOrchard_text).add(new FarmableBasicFruit(mustardCropBlock, 7));
		}
		...
		for (String cropnutName : cropNuts) {
			...
			if (PluginManager.Module.FARMING.isEnabled() && cropnutSeed != null && cropnutBlock != null) {
				Farmables.farmables.get(farmWheat_text).add(new FarmableGenericCrop(cropnutSeed, cropnutBlock, 7));
				Farmables.farmables.get(farmOrchard_text).add(new FarmableBasicFruit(cropnutBlock, 7));
			}
			if (cropnut != null) {
		...
		for (String nutName : nuts) {
			...
			if (PluginManager.Module.FARMING.isEnabled() && nutBlock != null) {
				Farmables.farmables.get(farmOrchard_text).add(new FarmableBasicFruit(nutBlock, 2));
			}
			if (nut != null) {
